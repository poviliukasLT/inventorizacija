...
if csv_file and xlsx_file:
    try:
        # Bandom keliais encoding'ais
        try:
            df_csv = pd.read_csv(csv_file, encoding='utf-8')
        except UnicodeDecodeError:
            df_csv = pd.read_csv(csv_file, encoding='windows-1257')

        if df_csv.empty:
            st.error("CSV failas tuščias arba netinkamai suformatuotas.")
        else:
        ...
        st.error("Nepavyko perskaityti CSV failo – failas galimai tuščias arba netinkamas formatas.")
